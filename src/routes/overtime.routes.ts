...
router.post("/", auth, async (req: Request, res: Response) => {
  try {
    ...
    // Check duplicate (compound unique: userId + date)
...
router.get("/pending", auth, async (req: Request, res: Response) => {
  try {
    ...
    if (!user || user.role !== "MANAGER") {
      return res
        .status(403)
        .json({ error: "Only managers can view pending requests" });
    }
...
router.patch("/:id/status", auth, async (req: Request, res: Response) => {
  try {
    ...
    if (!user || user.role !== "MANAGER") {
      return res
        .status(403)
        .json({ error: "Only managers can approve/reject requests" });
    }
...
router.delete("/:id", auth, async (req: Request, res: Response) => {
  try {
    ...
    if (request.status !== "PENDING") {
      return res
        .status(400)
        .json({ error: "Cannot delete processed request" });
    }
